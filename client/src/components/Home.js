...
class Home extends React.Component {
  ...
  componentDidMount() {
    ...
    console.log("8888888888888888: ",  this.props.match.params.id)
    axios.get("/api/videos").then(response => {
      const allVideos = response.data;
      // Making sure that my uploaded video will be the video displayed in the main page
      console.log("dddddddddddddd: ",  this.props.match.params.id)
...
handleDelete = (id) =>{
  ...
  render() {
    ...
    return (
      <div>
        <Hero hero={this.state.main[0].image} id = {this.state.main[0].id} video = {this.state.main[0].video}  />
